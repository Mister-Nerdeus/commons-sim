...
| Critical | Live branch protection is absent for `develop`. | `artifacts/controls/branch-protection/develop.json` reports `Branch not protected`. | Repository administrator | Apply `develop` protection after the final evidence commit is pushed: `node scripts/controls/apply-branch-protection.mjs --repo Mister-Nerdeus/commons-sim --branches develop --enforce-admins`, then re-export evidence. |
