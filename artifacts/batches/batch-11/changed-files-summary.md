# Batch 11 Changed Files Summary

## Issue #31

- `docs/program/BRANCH_PROTECTION_POLICY.md`
- `docs/program/checklists/RC_CHECKLIST.md`
- `docs/program/checklists/PRODUCTION_APPROVAL_CHECKLIST.md`
- `.github/workflows/ci.yml`
- `.github/workflows/determinism.yml`
- `artifacts/controls/required-check-map.json`

## Issue #32

- `.github/workflows/determinism.yml`
- `.github/workflows/promote-to-staging.yml`
- `.github/workflows/release-prod.yml`
- `.github/workflows/deploy-dev.yml`
- `.github/workflows/deploy-staging.yml`
- `.github/workflows/deploy-prod.yml`
- `.github/workflows/test.yml`
- `.github/workflows/verify-env-parity.yml`
- `Dockerfile`
- `Dockerfile.api`
- `docs/setup/LOCAL_SETUP.md`
- `docs/program/WORKFLOW_RUNTIME_POLICY.md`

## Issue #33

- `scripts/controls/export-branch-protection.mjs`
- `scripts/controls/export-environments.mjs`
- `docs/program/LIVE_CONTROL_EVIDENCE.md`
- `artifacts/controls/branch-protection/*.json`
- `artifacts/controls/environments/*.json`

## Issue #34

- `scripts/data/capture-staging-baseline.mjs`
- `scripts/data/reset-staging-from-baseline.mjs`
- `docs/data/STAGING_DATA_POLICY.md`
- `docs/data/STAGING_REFRESH_RUNBOOK.md`
- `docs/data/STAGING_RESET_CONTRACT.md`
- `artifacts/data/staging-baseline/*`

## Issue #35

- `docs/audits/BATCH_11_AUDIT.md`
- `artifacts/batches/batch-11/*`
