...
Local validation is the replacement merge gate for GitHub Actions. Every closeout evidence bundle must include a current `artifacts/controls/local-validation/latest.json` generated by `scripts/controls/run-local-validation.mjs`.

The local validation runner now fails when the active Node or pnpm major version does not match root `package.json` engines. It also fails when the validation evidence SHA does not match the current `HEAD`.
